//
//  PrizmSetCreationViewController.swift
//  FlashPrizm
//
//  Created by Michelle Kwan on 2/20/23.
//

import UIKit

class PrizmSetCreationViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate {
    
    var prizms = UITableView()
    var prizmTitle = "Title"
    var prizmDescription = "Description"
    var prizmCategory = "Category"
...
//    
    func setUpTableData() {
        ...
        // by default there is 1 term, 1 side
        prizmSetContents.append(["Title"])
        prizmSetContents.append(["Term 1", "Side 1"])
    ...
    func setUpNavBar() {
        ...
        navigationItem.title = "Create PrizmSet"
        
        navigationItem.rightBarButtonItem = UIBarButtonItem(title: "Create", style: .plain, target: self, action: #selector(doneAction))
    }
    ...
    func setUpTableView() {
        ...
        prizms.rowHeight = 60
        //prizms.rowHeight = prizms.frame / 2
...
//    }
    ...
    @objc func doneAction(sender: UIButton!) {
        ...
        for termIndex in 1..<prizmSetContents.count-1 {
            ...
            let runningIndex = termIndex - 1
            if runningIndex >= runningOrder.count {
                runningOrder.append(title)
            } else {
                runningOrder[runningIndex] = title
            }
        ...
        FirebaseManager.shared.createPrizm(prizmName: prizmTitle, description: prizmDescription, className: prizmCategory, prizmDic: prizmContents, prizmOrder: runningOrder) { error in
